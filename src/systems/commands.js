...
const commandRegistry = {
    ...
    // --- 殺敵 (Kill) 與 切磋 (Fight) ---
    // 共用戰鬥邏輯，isLethal 決定是否為死鬥
    'kill': {
        description: '下殺手',
        execute: async (playerData, args, userId) => {
            startCombat(playerData, args, userId, true);
        }
    },
    'fight': {
        description: '切磋',
        execute: async (playerData, args, userId) => {
            startCombat(playerData, args, userId, false);
        }
    },
    ...
    'recall': { 
        ...
        execute: (p,a,u) => {
            // [新增] 鬼門關禁止使用 recall
            if (p.location === "ghost_gate") {
                UI.print("鬼門關豈是你想來就來，想走就走的？", "error");
                return;
            }
...
// --- 通用戰鬥啟動函式 ---
async function startCombat(playerData, args, userId, isLethal) {
            ...
            if (isHit) {
                ...
                if (currentCombatState.npcHp <= 0) {
                    ...
                    if (!isLethal) {
                        ...
                        UI.print(UI.txt(`切磋勝利！獲得少許潛能。`, "#00ff00", true), "system", true);
            ...
            if (nIsHit) {
                ...
                if (playerData.attributes.hp <= 0) {
                    ...
                    if (!isLethal) {
                        ...
                        playerData.isUnconscious = true; // 設為昏迷，等待自動回復喚醒
                        stopCombat(userId);
...
export const CommandSystem = {
    handle: (inputStr, playerData, userId) => {
        ...
        // [新增] 自動甦醒機制
        // 如果玩家處於昏迷狀態 (isUnconscious)，但血量已經回復 (>0)，則自動醒來
