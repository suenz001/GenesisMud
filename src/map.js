    ...
    look: async (playerData) => {
        ...
        try {
            ...
            activeSnaps.forEach(doc => {
                const data = doc.data();
                // 3分鐘沒更新則自動清理
                if (Date.now() - (data.lastCombatTime || 0) > 180000) {
                     deleteDoc(doc.ref);
                } else {
                ...
                if (!isDead) {
                    ...
                    if (npc) {
                        ...
                        if (activeNpcMap.has(uniqueId)) {
                            ...
                            isUnconscious = activeData.isUnconscious === true || activeData.currentHp <= 0;
    ...
    lookTarget: async (playerData, targetIdOrName) => {
        ...
        if (room.npcs) {
            ...
            for (let i = 0; i < room.npcs.length; i++) {
                ...
                if (nid === targetIdOrName || NPCDB[nid].name === targetIdOrName) {
                    targetNpcId = nid;
                    realIndex = i;
                    break;
                }
            ...
            if (targetNpcId) {
                ...
                try {
                    const deadRef = collection(db, "dead_npcs");
                    const q = query(deadRef, where("roomId", "==", playerData.location), where("npcId", "==", targetNpcId), where("index", "==", realIndex));
                    const snapshot = await getDocs(q);
                    if (!snapshot.empty) {
                        const data = snapshot.docs[0].data();
                        if (Date.now() < data.respawnTime) isDead = true;
                    }
                } catch(e) { console.error(e); }
                ...
                try {
                    ...
                    if (activeSnap.exists()) {
                        ...
                        // 明確檢查昏迷狀態或血量歸零
                        isUnconscious = activeData.isUnconscious === true || displayHp <= 0;
                    }
                } catch (e) {
